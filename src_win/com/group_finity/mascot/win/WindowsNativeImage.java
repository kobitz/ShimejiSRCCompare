...
class WindowsNativeImage implements NativeImage {
	...
	* @ Param nativeHandle bitmap handle.
	...
	/**
	 * Creates a WindowsNativeImage pre-allocated at the given dimensions,
	 * filled with transparent pixels. Use updatePixels() to write content.
	 * This avoids CreateDIBSection on every scale change.
	 */
	public static WindowsNativeImage createEmpty( final int width, final int height )
	{
		BufferedImage empty = new BufferedImage( width, height, BufferedImage.TYPE_INT_ARGB_PRE );
		return new WindowsNativeImage( empty );
	}

	@Override
	public void updatePixels( final java.awt.image.BufferedImage src )
	{
		final int allocW = managedImage.getWidth( );
		final int allocH = managedImage.getHeight( );
		final int srcW   = Math.min( src.getWidth( ),  allocW );
		final int srcH   = Math.min( src.getHeight( ), allocH );
		final int[] rgb  = new int[ allocW * allocH ];
		final int[] srcPixels = src.getRGB( 0, 0, srcW, srcH, null, 0, srcW );
		for( int y = 0; y < srcH; y++ )
			System.arraycopy( srcPixels, y * srcW, rgb, y * allocW, srcW );
		flushNative( this.getNativeHandle( ), rgb );
	}
